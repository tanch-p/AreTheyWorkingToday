...
const parseData = (data) => {
    return data.map((holidayData, index) => {
        if(holidayData.type[0] === "National holiday"){
            return ({countryId:holidayData.country.id, name:holidayData.name, description:holidayData.description, $D:holidayData.date.datetime.day, $M:holidayData.date.datetime.month-1, $y:holidayData.date.datetime.year, span:<span className='orange'></span>})
        }
    })
}
