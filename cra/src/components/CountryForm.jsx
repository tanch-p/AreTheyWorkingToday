...
const CountryForm = ({ countriesData }) => {
	...
	const selectedCountries = selectedList.map((country, index) => (
		<li onClick={handleListClick} key={country}>
			{country} <span className="day day-events blue"></span>
		</li>
	));
		...
		<>
			<div className="form-container grid grid-cols-7 row-span-2 mx-auto border-4 border-gray-400">
				...
				<div className="grid grid-rows-2 col-span-1">
					<button name=">" disabled={false} onClick={handleAdd}>
						Add
					</button>
					<button name="<" disabled={false} onClick={handleRemove}>
						Remove
					</button>
				</div>
				<div className="col-span-3">
